...
import type { FormSchema, FormActionType } from '../types/index';
import type { FormProps } from '../props';
import { unref, toRaw } from 'vue';
import { isFunction, isNullOrUnDef } from '../utils/is';

export type EmitType = (event: 'reset' | 'submit' | 'register', ...args: any[]) => void;
...
export function useFormEvents({
  ...
  async function resetFields(): Promise<void> {
    ...
    Object.keys(formModel).forEach((key) => {
      ...
      formModel[key] = isNullOrUnDef(unref(defaultFormModel)[key]) ? null : unref(defaultFormModel)[key];
    });
    await clearValidate();
